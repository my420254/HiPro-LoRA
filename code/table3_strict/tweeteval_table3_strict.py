...
CONFIGS = {
    1000: {
        ...
        "eval_steps": 15, "memory_size": 200, "temperature": 0.12, "loss_weight": 0.001,    
        "warmup_steps": 15, "tail_weight": 2.0, "lr_scale": 1.0, "grad_acc": 1,                
        "fusion_init": -4.5, "smoothing": 0.05, "clamp_weights": False
    },
    2000: {
        "train": {1: 1200, 2: 600, 0: 200}, 
        "eval_steps": 10, "memory_size": 200, "temperature": 0.2, "loss_weight": 0.005,        
        "warmup_steps": 30, "tail_weight": 2.0, "lr_scale": 1.0, "grad_acc": 1,                
        "fusion_init": -3.0, "smoothing": 0.05, "clamp_weights": True        
    }
}

TAIL_CLASSES = [0, 2] # 0: Negative, 2: Positive (Neutral is head)

# ==================== [FINAL EXPERIMENT LIST: 7 Experiments] ====================
EXPERIMENTS = [{'name': 'AHSP-OnlyMax', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': True, 'memory_bank': True, 'use_max': True, 'use_mean': False, 'use_attn': False}, {'name': 'AHSP-OnlyMean', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': True, 'memory_bank': True, 'use_max': False, 'use_mean': True, 'use_attn': False}, {'name': 'AHSP-OnlyAttn', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': True, 'memory_bank': True, 'use_max': False, 'use_mean': False, 'use_attn': True}, {'name': 'AHSP-Max+Mean', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': True, 'memory_bank': True, 'use_max': True, 'use_mean': True, 'use_attn': False}, {'name': 'AHSP-Max+Attn', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': True, 'memory_bank': True, 'use_max': True, 'use_mean': False, 'use_attn': True}, {'name': 'AHSP-Mean+Attn', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': True, 'memory_bank': True, 'use_max': False, 'use_mean': True, 'use_attn': True}, {'name': 'No-AHSP', 'method': 'peft', 'loss_type': 'original', 'use_class_weight': True, 'peft_type': 'lora', 'hsp': False, 'memory_bank': True}]
...
ARTIFACT_DIR = os.path.join(SCRIPT_DIR, "artifacts", "tweeteval")
os.makedirs(RESULTS_DIR, exist_ok=True)
os.makedirs(ARTIFACT_DIR, exist_ok=True)
MAIN_RESULTS_FILE = os.path.join(SCRIPT_DIR, "results", "tweeteval_table3_strict_results.csv")
SENSITIVITY_FILE = os.path.join(SCRIPT_DIR, "results", "tweeteval_table3_strict_sensitivity_unused.csv")
...
class UnifiedModel(nn.Module):
    def __init__(self, cfg):
        ...
        if not self.is_peft: self.model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=NUM_LABELS); self.config = self.model.config
            ...
            if cfg.get("hsp", False): self.hsp_module = HierarchicalSmartPooling(hs, dr=0.1, use_max=cfg.get("use_max", True), use_mean=cfg.get("use_mean", True), use_attn=cfg.get("use_attn", True)); self.classifier_hsp = nn.Linear(hs, NUM_LABELS); nn.init.normal_(self.classifier_hsp.weight, std=0.02); nn.init.zeros_(self.classifier_hsp.bias); self.fusion_weight = nn.Parameter(torch.tensor([cfg.get("fusion_init", 0.1)]))
...
print(f"\n{'='*80}\nPART A: STRICT HELD-OUT ABLATION EXPERIMENTS\n{'='*80}")
for N_SAMPLES in [1000, 2000]: 
    ...
    for exp in EXPERIMENTS:
        ...
        for seed_idx, SEED in enumerate(RANDOM_SEEDS):
            ...
            output_dir_path = os.path.join(ARTIFACT_DIR, f"tmp_tweeteval_{N_SAMPLES}_{safe_name}_{SEED}")
