...
PEFT_LR = 3e-4        
# 3           2   0        4      1          5
# Neutral    Fear Angry  Happy   Sad    Surprise 
# Dataset Specific Configs
CONFIGS = {
    1000: {
        "train": {3: 450, 2: 250, 0: 120, 4: 100, 1: 60, 5: 20},
        "eval_steps": 10, "memory_size": 1200, "temperature": 0.2, "loss_weight": 0.2,      
        "warmup_steps": 5, "tail_weight": 4.0, "lr_scale": 1.0, "grad_acc": 2,              
        "fusion_init": -2.0, "smoothing": 0.1, "clamp_weights": True    
    },
    2000: {
...
ARTIFACT_DIR = os.path.join(SCRIPT_DIR, "artifacts", "smp2020")
os.makedirs(RESULTS_DIR, exist_ok=True)
os.makedirs(ARTIFACT_DIR, exist_ok=True)
MAIN_RESULTS_FILE = os.path.join(SCRIPT_DIR, "results", "smp2020_table3_strict_results.csv")
SENSITIVITY_FILE = os.path.join(SCRIPT_DIR, "results", "smp2020_table3_strict_sensitivity_unused.csv")
...
class UnifiedModel(nn.Module):
    def __init__(self, cfg):
        ...
        if not self.is_peft: self.model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=NUM_LABELS); self.config = self.model.config
            ...
            if cfg.get("hsp", False): self.hsp_module = HierarchicalSmartPooling(hs, dr=0.1, use_max=cfg.get("use_max", True), use_mean=cfg.get("use_mean", True), use_attn=cfg.get("use_attn", True)); self.classifier_hsp = nn.Linear(hs, NUM_LABELS); nn.init.constant_(self.classifier_hsp.weight, 0); nn.init.constant_(self.classifier_hsp.bias, 0); self.fusion_weight = nn.Parameter(torch.tensor([cfg.get("fusion_init", 0.1)]))
