...
BASE_DIR = SCRIPT_DIR
MAIN_RESULTS_FILE = os.path.join(SCRIPT_DIR, "results", "sst5_sensitivity_main.csv")
SENSITIVITY_FILE = os.path.join(SCRIPT_DIR, "results", "sst5_sensitivity.csv")
GATE_LOG_DIR      = os.path.join(SCRIPT_DIR, "gate_logs", "sst5_sensitivity")   # ✅ 门控过程日志目录
IMG_DATA_DIR = os.path.join(SCRIPT_DIR, "viz", "sst5_sensitivity")
...
for N in [2300]:
    ...
    for exp in EXPERIMENTS:
        ...
        for seed in RANDOM_SEEDS:
            ...
            output_dir_path = f"./tmp_sst5_sensitivity_sst5_{N}_{safe_name}_{seed}"
    ...
    for val in values:
        for seed in RANDOM_SEEDS:
            ...
            trainer = UnifiedTrainer(
                ...
                cls_num_list=[len(train_df[train_df['label'] == i]) for i in range(NUM_LABELS)],
                memory_loss=MemoryBank(
                    128, NUM_LABELS, cur_mem, cur_temp,
                ...
                compute_metrics=compute_metrics,
                args=TrainingArguments(
                    output_dir=f"./tmp_sst5_sensitivity_sens_{param_name}_{val}_{seed}",
                    num_train_epochs=EPOCHS,
                    per_device_train_batch_size=BATCH_SIZE,
            ...
            del model, trainer
            torch.cuda.empty_cache(); gc.collect()
            shutil.rmtree(f"./tmp_sst5_sensitivity_sens_{param_name}_{val}_{seed}", ignore_errors=True)


run_sens_sst5("MemSize",     SENS_MEM_SIZES)
run_sens_sst5("TailWeight",  SENS_TAIL_WEIGHTS)
run_sens_sst5("Temperature", SENS_TEMPS)
run_sens_sst5("LossWeight",  SENS_LOSS_WEIGHTS)
