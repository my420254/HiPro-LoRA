...
GATE_LOG_DIR = os.path.join(SCRIPT_DIR, "gate_logs", "tweeteval")
TABLE2_REF_FILE = os.path.join(PROJECT_ROOT, "table2_strict", "results", "tweeteval_main_table2_strict_results.csv")
TARGET_N = int(os.environ.get("HIPRO_GATE_N", "1000"))
...
MAIN_RESULTS_FILE = os.path.join(RESULTS_DIR, f"tweeteval_gate_N{TARGET_N}_seed{TARGET_SEEDS[0] if len(TARGET_SEEDS) == 1 else 'all'}.csv")
...
print(f">>> Gate run: dataset=TweetEval | N={TARGET_N} | method={TARGET_METHOD} | seeds={TARGET_SEEDS}")
...
for N_SAMPLES in [TARGET_N]: 
    ...
    for exp in EXPERIMENTS:
        ...
        for seed_idx, SEED in enumerate(TARGET_SEEDS):
            ...
            output_dir_path = f"./tmp_tweeteval_gate_{N_SAMPLES}_{safe_name}_{SEED}"
            gate_log_file = os.path.join(GATE_LOG_DIR, f"tweeteval_N{N_SAMPLES}_{safe_name}_seed{SEED}_gate.csv")
            ...
            trainer = UnifiedTrainer(
                ...
                train_dataset=train_ds, eval_dataset=val_ds, tokenizer=tokenizer, data_collator=DataCollatorWithPadding(tokenizer), compute_metrics=compute_metrics, callbacks=[EarlyStoppingCallback(early_stopping_patience=8), GateTraceCallback("TweetEval", N_SAMPLES, exp['name'], SEED, gate_log_file)], 
            ...
            append_gate_snapshot(gate_log_file, model, "TweetEval", N_SAMPLES, exp['name'], SEED, "best_model_after_predict", trainer.state, res)
