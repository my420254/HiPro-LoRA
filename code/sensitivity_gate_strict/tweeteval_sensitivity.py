...
BASE_DIR = SCRIPT_DIR
MAIN_RESULTS_FILE = os.path.join(SCRIPT_DIR, "results", "tweeteval_sensitivity_main.csv")
SENSITIVITY_FILE = os.path.join(SCRIPT_DIR, "results", "tweeteval_sensitivity.csv")
GATE_LOG_DIR      = os.path.join(SCRIPT_DIR, "gate_logs", "tweeteval_sensitivity")   # ✅ 门控过程日志目录
IMG_DATA_DIR = os.path.join(SCRIPT_DIR, "viz", "tweeteval_sensitivity")
...
def run_sens(param_name, values):
    for val in values:
        for SEED in RANDOM_SEEDS:
            ...
            output_dir    = f"./tmp_tweeteval_sensitivity_sens_{param_name}_{val}_{SEED}"
